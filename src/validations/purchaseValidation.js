const validatePurchaseCreate = (req) => {
  const { supplierName, orderDate, stockItemId, quantity, unitPrice, paymentTerm, status, currency } = req.body || {};
  const errors = [];

  if (!supplierName || supplierName.trim().length === 0) {
    errors.push('Tedarikçi ticari unvanı zorunludur.');
  }
  ...
  if (!stockItemId || String(stockItemId).trim().length === 0) {
    errors.push('Malzeme / Stok kartı seçilmesi zorunludur.');
  }
  ...
  if (isNaN(parsedPrice) || parsedPrice < 0) {
    errors.push('Birim alış fiyatı negatif olamaz.');
  }
  ...
  const validStatuses = ['Draft', 'Pending_Approval', 'Ordered', 'Partial_Received', 'Received', 'Cancelled'];
  if (!status || !validStatuses.includes(status)) {
    errors.push('Geçerli bir satın alma sipariş durumu seçiniz.');
  }
...
module.exports = { validatePurchaseCreate };
